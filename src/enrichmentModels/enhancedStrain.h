...
#include "../../include/userInputParameters_cp.h"
...
public:
   ...
   */
  enhancedStrain(const FiniteElement<dim, dim> &fe, ConditionalOStream  pcout_temp, userInputParameters_cp _userInputs_cp);
  /**
   *Class destructor.
   */
  ~enhancedStrain();
...
template <int dim>
enhancedStrain<dim>::enhancedStrain(const FiniteElement<dim, dim> &fe, ConditionalOStream  pcout_temp, userInputParameters_cp _userInputs_cp)
  :
  center_quad(1),
  quad_formula(_userInputs_cp.quadOrder),
