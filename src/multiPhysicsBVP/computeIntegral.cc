...
void  MultiPhysicsBVP<dim,degree>::computeIntegral(double& integratedField, int index, std::vector<vectorType_pf*> variableSet) {
...
template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::computeIntegralMF(double& integratedField, int index, const std::vector<vectorType_pf*> variableSet){
  //log time
  computing_timer_pf.enter_subsection("multiPhysicsBVP: computeIntegralMF");

  integrated_var = 0.0;
  integral_index = index;

  //call to integrate and assemble
  matrixFreeObject.cell_loop (&MultiPhysicsBVP<dim,degree>::getIntegralMF, this, residualSet, variableSet);

  integratedField=Utilities::MPI::sum(integrated_var, MPI_COMM_WORLD);

  //end log
  computing_timer_pf.leave_subsection("multiPhysicsBVP: computeIntegralMF");
}

template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::getIntegralMF(const MatrixFree<dim,double> &data,
