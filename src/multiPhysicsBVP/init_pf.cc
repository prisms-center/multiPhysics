// init_pf() method for MultiPhysicsBVP class
...
template <int dim, int degree>
 void MultiPhysicsBVP<dim,degree>::init_pf(){
	 computing_timer_pf.enter_subsection("multiPhysicsBVP: initialization");

	 //creating mesh

	 pcout << "creating problem mesh...\n";
     // Create the coarse mesh and mark the boundaries
     makeTriangulation(triangulation_pf);

	 // Set which (if any) faces of the triangulation_pf are periodic
	 setPeriodicity_pf();

     // If resuming from a checkpoint, load the refined triangulation_pf, otherwise refine globally per the parameters.in file
     if (userInputs_pf.resume_from_checkpoint){
         load_checkpoint_triangulation();
     }
     else {
         // Do the initial global refinement
    	 triangulation_pf.refine_global (userInputs_pf.refine_factor);
     }


	 // Write out the size of the computational domain and the total number of elements
     if (dim < 3){
         pcout << "problem dimensions: " << userInputs_pf.domain_size[0] << "x" << userInputs_pf.domain_size[1] << std::endl;
     }
     else {
         pcout << "problem dimensions: " << userInputs_pf.domain_size[0] << "x" << userInputs_pf.domain_size[1] << "x" << userInputs_pf.domain_size[2] << std::endl;
     }
	 pcout << "number of elements: " << triangulation_pf.n_global_active_cells() << std::endl;
	 pcout << std::endl;
	 ...
	 for(typename std::vector<Field<dim> >::iterator it = fields.begin(); it != fields.end(); ++it){
		 ...
		 if (it->type==SCALAR){
			 ...
			 pcout << "\nmultiPhysicsBVP.h: unknown field type\n";
			 exit(-1);
		 }
		 FESet.push_back(fe);
		 ...
		 setPeriodicityConstraints_pf(constraintsOther,dof_handler);

     // Check if Dirichlet BCs are used
     has_Dirichlet_BCs = false;
     for (unsigned int i=0; i<userInputs_pf.BC_list.size(); i++){
         for (unsigned int direction = 0; direction < 2*dim; direction++){
             if (userInputs_pf.BC_list[i].var_BC_type[direction] == DIRICHLET || userInputs_pf.BC_list[i].var_BC_type[direction] == NON_UNIFORM_DIRICHLET){
	 ...
	 for(unsigned int fieldIndex=0; fieldIndex<fields.size(); fieldIndex++){
		 ...
		 if (fields[fieldIndex].pdetype==TIME_INDEPENDENT || fields[fieldIndex].pdetype==IMPLICIT_TIME_DEPENDENT || (fields[fieldIndex].pdetype==AUXILIARY && userInputs_pf.var_nonlinear[fieldIndex])){
	 ...
	 soltransSet.clear();
	 for(unsigned int fieldIndex=0; fieldIndex<fields.size(); fieldIndex++){
		 soltransSet.push_back(new parallel::distributed::SolutionTransfer<dim, vectorType_pf>(*dofHandlersSet_nonconst[fieldIndex]));
	 }
	 ...
	 computing_timer_pf.leave_subsection("multiPhysicsBVP: initialization");
}

template <int dim, int degree>
 void MultiPhysicsBVP<dim,degree>::makeTriangulation(parallel::distributed::Triangulation<dim> & tria) const{
     if (dim == 3){
    	 GridGenerator::subdivided_hyper_rectangle (tria, userInputs_pf.subdivisions, Point<dim>(), Point<dim>(userInputs_pf.domain_size[0],userInputs_pf.domain_size[1],userInputs_pf.domain_size[2]));
     }
     else if (dim == 2){
    	 GridGenerator::subdivided_hyper_rectangle (tria, userInputs_pf.subdivisions, Point<dim>(), Point<dim>(userInputs_pf.domain_size[0],userInputs_pf.domain_size[1]));
     }
     else {
    	 GridGenerator::subdivided_hyper_rectangle (tria, userInputs_pf.subdivisions, Point<dim>(), Point<dim>(userInputs_pf.domain_size[0]));
     }

     // Mark boundaries for applying the boundary conditions
	 markBoundaries_pf(tria);
