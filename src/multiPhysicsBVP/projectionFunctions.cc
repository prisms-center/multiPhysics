...
void MultiPhysicsBVP<dim,degree>::initProjection(){
  ...
  const unsigned int num_local_cells = triangulation_cp.n_locally_owned_active_cells();
...
template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::projection(){

  //return if no post processing fields
 // if (!userInputs_cp.writeOutput) return;//cp
 // if (numPostProcessedFields==0) return;//cp
...
if (userInputs_cp.tabularOutput){
 ...
 /* if (((!userInputs_cp.tabularOutput)&&((currentIncrement_cp+1)%userInputs_cp.skipOutputSteps!=0))||((userInputs_cp.tabularOutput)&& (std::count(tabularTimeInputIncInt.begin(), tabularTimeInputIncInt.end(), (currentIncrement_cp+1))==0))){
    return;
  }*/
//cp
