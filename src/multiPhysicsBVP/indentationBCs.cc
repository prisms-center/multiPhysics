...
//void MultiPhysicsBVP<dim,degree>::setIndentation(){
...
//}
template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::meshRefineIndentation() {
    ...
    {
        centerPlasticZone(i)= userInputs_cp.refinementCenter[i];
    }
    for (unsigned int radius_j = 0; radius_j < userInputs_cp.refinementFactor.size(); ++radius_j)
    {
        for (unsigned int step = 0; (int)step < userInputs_cp.refinementFactor[radius_j]; ++step)
        {
            for (auto &cell : triangulation_cp.active_cell_iterators())
            {
...
//                            centerPlasticZone.distance(cell->vertex(v));
                    ...
                    if (distance_from_center <= userInputs_cp.refinementZoneSize[radius_j])
                    {
            ...
            triangulation_cp.execute_coarsening_and_refinement();
        }
    }
...
template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::updateIndentPos() {
    double pos;
    unsigned int nextFrame, prevFrame;
    for (unsigned int j = 0; j < userInputs_cp.indentationKeyFrames; ++j){
        if (userInputs_cp.indentationKeyFrameTimeIncs[j] > (int)currentIncrement_cp){
            nextFrame = j;
            prevFrame = j-1;
            break;
        }
    ...
    pos = (currentIncrement_cp + 1 - userInputs_cp.indentationKeyFrameTimeIncs[prevFrame]);
    pos = pos/(userInputs_cp.indentationKeyFrameTimeIncs[nextFrame] -
            userInputs_cp.indentationKeyFrameTimeIncs[prevFrame]) ;
    for (int i = 0; i < dim; ++i) {
...
template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::displaceNode(const Point<dim> & p, const Point<dim> & u)
{
...
//INDENTATION
template <int dim, int degree>
bool MultiPhysicsBVP<dim,degree>::flagActiveSet(const Point<dim> & p)
{
    double dist2;
    double dist1;
    double tmp;
...
template <int dim, int degree>
bool MultiPhysicsBVP<dim,degree>::flagActiveSetLambda(const Point<dim> & p, double & criterion)
{
    double dist2;
    double dist1;
    double tmp;
...
template <int dim, int degree>
bool MultiPhysicsBVP<dim,degree>::flagActiveSetLambda2(const Point<dim> & p, double & criterion)
{
    double dist2;
    double dist1;
    double tmp;
...
template <int dim, int degree>
double MultiPhysicsBVP<dim,degree>::Obstacle(const Point<dim> & p, const unsigned int & component, const std::vector<double> & ind)
{
...
template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::assemble_mass_matrix_diagonal()
{
    ...
    for (const auto &cell : dofHandler.active_cell_iterators())
        if (cell->is_locally_owned()) {
            ...
            for (const auto &face: cell->face_iterators())
                if (face->at_boundary() && (face->boundary_id() == indenterFace || userInputs_cp.readExternalMesh)) {
...
/*template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::updateActiveSet(){
//    pcout << "in updateActiveSet\n";
    ...
    vectorType_cp distributed_solution(locally_owned_dofs,
                                        mpi_communicator);
//    pcout << "in updateActiveSet 1\n";
    distributed_solution = solution;
    vectorType_cp lambda(locally_relevant_dofs,
                          mpi_communicator);
//    pcout << "in updateActiveSet 2\n";
    lambda = newton_rhs_uncondensed; //NEED THIS COMPUTED PRIOR TO CALL Done
    vectorType_cp diag_mass_matrix_vector_relevant(
...
//                                    obstacle->value(this_support_point, 2);
                            ...
                            const double c = 100.0 * userInputs_cp.elasticStiffness1[0][0]; //e_modulus;
...
//INDENTATION
template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::setIndentation(const Point<dim>& node,
...
template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::setIndentation2(const Point<dim>& node,
                                      const unsigned int dof,
...
template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::setActiveSet(){
    std::vector<bool> dof_touched(dofHandler.n_dofs(), false);
    ...
    for (; cell!=endc; ++cell) {
        if (cell->is_locally_owned()){
            ...
            for (unsigned int faceID=0; faceID<GeometryInfo<dim>::faces_per_cell; faceID++){ //(const auto &face: cell->face_iterators()){
                if (cell->face(faceID)->at_boundary() && (cell->face(faceID)->boundary_id() == indenterFace )){ //|| userInputs_cp.readExternalMesh && cell->face(faceID)->boundary_id()==indenterFace){ //(face->at_boundary() && face->boundary_id()==indenterFace) {
                    ...
                    if (fe_face_values.shape_value(i, 0) != 0) {
                        ...
                        if ((dof == indentDof) || (roughIndenter == true)) {
                            if (dof_touched[globalDOF] == false) {
                                ...
                                flag = false;
                            if (flag) {
                                if (userInputs_cp.debugIndentation && own_dofs.is_element(globalDOF)) {
...
template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::measureIndentationLoad(){
    vectorType_cp lambda2(locally_owned_dofs,locally_relevant_ghost_dofs,mpi_communicator);
    std::vector<bool> dof_touched(dofHandler.n_dofs(), false);
    const unsigned int   dofs_per_cell   = FE.dofs_per_cell;
    std::vector<types::global_dof_index> local_dof_indices (dofs_per_cell);
    ...
    vectorType_cp diag_mass_matrix_vector_relevant(locally_owned_dofs,
    ...
    for (; cell!=endc; ++cell) {
        if (cell->is_locally_owned()){
            ...
            fe_values.reinit (cell);
            for (unsigned int faceID=0; faceID<GeometryInfo<dim>::faces_per_cell; faceID++){ //(const auto &face: cell->face_iterators()){
                if (cell->face(faceID)->at_boundary() && (cell->face(faceID)->boundary_id()==loadFace)){ //|| userInputs_cp.readExternalMesh //(face->at_boundary() && face->boundary_id()==indenterFace) {
                    fe_face_values.reinit(cell, faceID); //face);
                    for (unsigned int i = 0; i < dofs_per_cell; ++i) {
                        if (fe_face_values.shape_value(i, 0) != 0) {
                            const unsigned int dof = fe_values.get_fe().system_to_component_index(i).first;
...
template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::setActiveSet2(){
    vectorType_cp lambda2(locally_owned_dofs,locally_relevant_ghost_dofs,mpi_communicator);
    std::vector<bool> dof_touched(dofHandler.n_dofs(), false);
...
//    std::vector<bool> dof_touched(dofHandler.n_dofs(), false);
    ...
    if (userInputs_cp.continuum_Isotropic)
        stiffness = (userInputs_cp.lame_lambda + 4/3 * userInputs_cp.lame_mu);
    else
        stiffness = userInputs_cp.elasticStiffness1[0][0];
    const double c = userInputs_cp.activeSetCriterionCoefficient*stiffness;
    typename DoFHandler<dim>::active_cell_iterator cell = dofHandler.begin_active(), endc = dofHandler.end();
    for (; cell!=endc; ++cell) {
        if (cell->is_locally_owned()){
            cell->get_dof_indices (local_dof_indices);
            fe_values.reinit (cell);
            for (unsigned int faceID=0; faceID<GeometryInfo<dim>::faces_per_cell; faceID++){ //(const auto &face: cell->face_iterators()){
                if (cell->face(faceID)->at_boundary() && (cell->face(faceID)->boundary_id()==indenterFace )){ //|| userInputs_cp.readExternalMesh(face->at_boundary() && face->boundary_id()==indenterFace) {
                    fe_face_values.reinit(cell, faceID); //face);
                    //cell->get_dof_indices (local_dof_indices);
                    ...
                    if (fe_face_values.shape_value(i, 0) != 0) {
                        ...
                        if ((dof == indentDof) || roughIndenter) {
                            ...
                            criterion = (lambda2(index_z) /
                                         diag_mass_matrix_vector_relevant(index_z) + userInputs_cp.activeSetLambdaTolerance +
...
template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::setFrozenSet(){
    vectorType_cp lambda2(locally_owned_dofs,locally_relevant_ghost_dofs,mpi_communicator);
    std::vector<bool> dof_touched(dofHandler.n_dofs(), false);
    bool active_set_empty = true;
    const unsigned int   dofs_per_cell   = FE.dofs_per_cell;
                    ...
                    for (unsigned int i = 0; i < dofs_per_cell; ++i) {
                        if (fe_face_values.shape_value(i, 0) != 0) {
                            ...
                            if ((dof == indentDof) || (roughIndenter == true)) {
                                criterion = (lambda2(index_z) /
                                             diag_mass_matrix_vector_relevant(index_z) +
                                             userInputs_cp.activeSetLambdaTolerance +
...
template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::setIndentationConstraints(){
