// Methods in MultiPhysicsBVP to update the list of nuclei
...
void MultiPhysicsBVP<dim,degree>::updateNucleiList() {

    if (userInputs_pf.nucleation_occurs){
        if (currentIncrement_pf % userInputs_pf.steps_between_nucleation_attempts == 0 || currentIncrement_pf == 1){
            if (userInputs_pf.dtValue*(double)currentIncrement_pf >= userInputs_pf.nucleation_start_time && userInputs_pf.dtValue*(double)currentIncrement_pf <= userInputs_pf.nucleation_end_time){
                computing_timer_pf.enter_subsection("multiPhysicsBVP: nucleation");
                // Apply constraints
                ...
                if (currentIncrement_pf == 1 && !userInputs_pf.evolution_before_nucleation){
                    while (new_nuclei.size() == 0){
                        currentTime_pf+=userInputs_pf.dtValue*(double)userInputs_pf.steps_between_nucleation_attempts;
                        currentIncrement_pf+=userInputs_pf.steps_between_nucleation_attempts;
                ...
                if (new_nuclei.size() > 0 && userInputs_pf.h_adaptivity == true){
                    refineMeshNearNuclei(new_nuclei);
                }
                computing_timer_pf.leave_subsection("multiPhysicsBVP: nucleation");
            }
        }
    }
}
...
// =======================================================================================================
template <int dim, int degree>
std::vector<nucleus<dim> > MultiPhysicsBVP<dim,degree>::getNewNuclei(){
    ...
    pcout << "Nucleation attempt for increment " << currentIncrement_pf << std::endl;

    getLocalNucleiList(newnuclei);
    pcout << "nucleation attempt! " << currentTime_pf << " " << currentIncrement_pf << std::endl;
    ...
    newnuclei = new_nuclei_parallel.buildGlobalNucleiList(userInputs_pf.min_distance_between_nuclei, nuclei.size());
...
// =================================================================================
template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::getLocalNucleiList(std::vector<nucleus<dim> > &newnuclei) const
{
    // Nickname for current time and time step
    double t=currentTime_pf;
    unsigned int inc=currentIncrement_pf;
    ...
    std::vector<std::vector<double> > var_values(userInputs_pf.nucleation_need_value.size(),std::vector<double>(num_quad_points));
    ...
    while (di != this->dofHandlersSet_nonconst[0]->end())
    {
        if (di->is_locally_owned()){
            ...
            fe_values.reinit(di);
            for (unsigned int var = 0; var < userInputs_pf.nucleation_need_value.size(); var++){
                fe_values.get_function_values(*(solutionSet[userInputs_pf.nucleation_need_value[var]]), var_values[var]);
            }
            ...
            for (unsigned int var = 0; var < userInputs_pf.nucleation_need_value.size(); var++){
                ...
                variable_values.set(userInputs_pf.nucleation_need_value[var],ele_val);
            }

            // Loop through each nucleating order parameter
            for (unsigned int i = 0; i < userInputs_pf.nucleating_variable_indices.size(); i++){
                unsigned int variable_index = userInputs_pf.nucleating_variable_indices.at(i);
                ...
                if (rand_val <= Prob){
                    ...
                    for (unsigned int j=0; j < dim; j++){
                        bool periodic_j = (userInputs_pf.BC_list[1].var_BC_type[2*j]==PERIODIC);
                        bool insafetyzone_j = (periodic_j || ((nuc_ele_pos[j] > userInputs_pf.get_no_nucleation_border_thickness(variable_index)) && (nuc_ele_pos[j] < userInputs_pf.domain_size[j]-userInputs_pf.get_no_nucleation_border_thickness(variable_index))));
                    ...
                    if (insafetyzone){
                        ...
                        for (unsigned int q_point=0; q_point<num_quad_points; ++q_point){
                            double sum_op = 0.0;
                            for (unsigned int var = 0; var < userInputs_pf.nucleation_need_value.size(); var++){
                                for (unsigned int op = 0; op < userInputs_pf.nucleating_variable_indices.size(); op++){
                                    if (userInputs_pf.nucleation_need_value[var] == userInputs_pf.nucleating_variable_indices[op]){
                                        sum_op += var_values[var][q_point];
                                    }
                                }
                            }
                            if (sum_op < userInputs_pf.nucleation_order_parameter_cutoff){
                                anyqp_OK =true;
                            }
                        }
                        ...
                        if (anyqp_OK){
                            ...
                            std::uniform_int_distribution<unsigned int> int_distr(0,userInputs_pf.nucleating_variable_indices.size()-1);
                            unsigned int op_for_nucleus = userInputs_pf.nucleating_variable_indices[int_distr(gen2)];
                            ...
                            */
                            ...
                            temp->semiaxes = userInputs_pf.get_nucleus_semiaxes(variable_index);
                            temp->seededTime=t;
                            temp->seedingTime = userInputs_pf.get_nucleus_hold_time(variable_index);
...
// =======================================================================================================
template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::safetyCheckNewNuclei(std::vector<nucleus<dim> > newnuclei, std::vector<unsigned int> &conflict_ids)
{
    //QGauss<dim>  quadrature(degree+1);
    QGaussLobatto<dim>  quadrature(degree+1);
    FEValues<dim> fe_values (*(FESet[0]), quadrature, update_values|update_quadrature_points|update_JxW_values);
    const unsigned int   num_quad_points = quadrature.size();
    std::vector<std::vector<double> > op_values(userInputs_pf.nucleating_variable_indices.size(),std::vector<double>(num_quad_points));
    std::vector<dealii::Point<dim> > q_point_list(num_quad_points);
    ...
    for (typename std::vector<nucleus<dim> >::iterator thisNucleus=newnuclei.begin(); thisNucleus!=newnuclei.end(); ++thisNucleus){
        ...
        while (di != dofHandlersSet_nonconst[0]->end())
        {
            if (di->is_locally_owned()){
                fe_values.reinit(di);
                for (unsigned int var = 0; var < userInputs_pf.nucleating_variable_indices.size(); var++){
                	fe_values.get_function_values(*(solutionSet[userInputs_pf.nucleating_variable_indices[var]]), op_values[var]);
                }
                q_point_list = fe_values.get_quadrature_points();
                ...
                for (unsigned int q_point=0; q_point<num_quad_points; ++q_point){
                    ...
                    double weighted_dist = weightedDistanceFromNucleusCenter(thisNucleus->center, userInputs_pf.get_nucleus_freeze_semiaxes(thisNucleus->orderParameterIndex), q_point_list[q_point], thisNucleus->orderParameterIndex);

                    if (weighted_dist < 1.0){
                    	double sum_op = 0.0;
                    	for (unsigned int num_op = 0; num_op < userInputs_pf.nucleating_variable_indices.size(); num_op++){
...
// =================================================================================
template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::refineMeshNearNuclei(std::vector<nucleus<dim> > newnuclei)
{
	//QGauss<dim>  quadrature(degree+1);
	QGaussLobatto<dim>  quadrature(degree+1);
	FEValues<dim> fe_values (*(FESet[0]), quadrature, update_values|update_quadrature_points|update_JxW_values);
	...
	for (unsigned int remesh_index=0; remesh_index < (userInputs_pf.max_refinement_level-userInputs_pf.min_refinement_level); remesh_index++){
		ti  = triangulation_pf.begin_active();
		...
		while (di != dofHandlersSet_nonconst[0]->end()){
			if (di->is_locally_owned()){
				...
				for (unsigned int i=0; i<dim; i++){
					diag_dist += (userInputs_pf.domain_size[i]*userInputs_pf.domain_size[i])/(userInputs_pf.subdivisions[i]*userInputs_pf.subdivisions[i]);
				}
				...
				for (unsigned int q_point=0; q_point<num_quad_points; ++q_point){
					for (typename std::vector<nucleus<dim> >::iterator thisNucleus=newnuclei.begin(); thisNucleus!=newnuclei.end(); ++thisNucleus){
						...
						if (weighted_dist < 1.0 || thisNucleus->center.distance(q_point_list[q_point]) < diag_dist){
							if ((unsigned int)ti->level() < userInputs_pf.max_refinement_level){
								mark_refine = true;
								break;
							}
						}
...
template <int dim, int degree>
double MultiPhysicsBVP<dim,degree>::weightedDistanceFromNucleusCenter(
    ...
    const unsigned int var_index) const
{
    ...
    for (unsigned int i=0; i<dim; i++){
        if (userInputs_pf.BC_list[var_index].var_BC_type[2*i]==PERIODIC){
            shortest_edist_tensor[i] = shortest_edist_tensor[i]-round(shortest_edist_tensor[i]/userInputs_pf.domain_size[i])*userInputs_pf.domain_size[i];
        }
    }
    shortest_edist_tensor = userInputs_pf.get_nucleus_rotation_matrix(var_index) * shortest_edist_tensor;
    for (unsigned int i=0; i<dim; i++){
...
template <int dim, int degree>
dealii::VectorizedArray<double> MultiPhysicsBVP<dim,degree>::weightedDistanceFromNucleusCenter(
    const dealii::Point<dim,double> center,
    const std::vector<double> semiaxes,
    ...
    const unsigned int var_index) const
{
    ...
    for (unsigned int j=0; j<dim; j++){
        ...
        if (userInputs_pf.BC_list[var_index].var_BC_type[2*j]==PERIODIC){
            for (unsigned k=0; k<q_point_loc(0).size();k++){
                shortest_edist_tensor[j][k] = shortest_edist_tensor[j][k]-round(shortest_edist_tensor[j][k]/userInputs_pf.domain_size[j])*userInputs_pf.domain_size[j];
            }
        }
    }
    shortest_edist_tensor = userInputs_pf.get_nucleus_rotation_matrix(var_index) * shortest_edist_tensor;
