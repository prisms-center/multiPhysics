                  ...
                  while (new_nuclei.size() == 0)
                    {
                      ...
                      currentIncrement +=
                        userInputs.steps_between_nucleation_attempts;
      ...
      if (di->is_locally_owned())
        {
          ...
          // quadrature points
          fe_values.reinit(di);
          for (unsigned int var = 0; var < userInputs.nucleation_need_value.size();
               var++)
            {
          ...
          // ---------------------------
          // NOTE: This might not be the best way to do this. This is missing the loop of
          // the DoFs from Step-3

          double             element_volume = 0.0;
          dealii::Point<dim> ele_center;
          // Loop over the quadrature points to find the element volume (or area in 2D)
          // and the average q point location
          ...
          // Loop over each variable and each quadrature point to get the average variable
          ...
          for (unsigned int i = 0; i < userInputs.nucleating_variable_indices.size();
               i++)
            {
              unsigned int variable_index =
                userInputs.nucleating_variable_indices.at(i);
              ...
              if (rand_val <= Prob)
                {
                  ...
                  // Finding coordinates of quadrature point closest to and furthest away
                  // from the origin
                  ...
                  if (insafetyzone)
                    {
                      // Check to see if the order parameter anywhere within the element
                      ...
                      if (anyqp_OK)
                        {
                          ...
                          std::cout << "Nucleation order parameter: " << op_for_nucleus <<
                          " " << rand_val << std::endl;
                          */
...
// =======================================================================================================
// Making sure all new nuclei from complete prospective list do not overlap with existing
// precipitates
// =======================================================================================================
template <int dim, int degree>
void
MatrixFreePDE<dim, degree>::safetyCheckNewNuclei(
  std::vector<nucleus<dim>>  newnuclei,
  std::vector<unsigned int> &conflict_ids)
{
  // QGauss<dim>  quadrature(degree+1);
                  ...
                  if (weighted_dist < 1.0)
                    {
                      ...
                      if (sum_op > 0.1)
                        {
                          isClose = true;
                          std::cout << "Attempted nucleation failed due to overlap w/ "
                                       "existing particle!!!!!!"
                                    << std::endl;
...
MatrixFreePDE<dim, degree>::refineMeshNearNuclei(std::vector<nucleus<dim>> newnuclei)
{
  ...
  for (unsigned int remesh_index = 0; remesh_index < (userInputs.max_refinement_level -
                                                      userInputs.min_refinement_level);
       remesh_index++)
    {
          ...
          if (di->is_locally_owned())
            {
              ...
              // Calculate the distance from the corner of the cell to the middle of the
              // cell
              double diag_dist = 0.0;
              for (unsigned int i = 0; i < dim; i++)
                {
                  diag_dist +=
                    (userInputs.domain_size[i] * userInputs.domain_size[i]) /
...
// First of two versions of this function to calculated the weighted distance from the
// center of a nucleus_semiaxes This version is for when the points are given as doubles
template <int dim, int degree>
double
...
// Second of two versions of this function to calculated the weighted distance from the
// center of a nucleus_semiaxes This version is for when the points are given as
// vectorized arrays
template <int dim, int degree>
