// reinit() method for MultiPhysicsBVP class

#include "../../include/multiPhysicsBVP.h"

 //populate with fields and setup matrix free system
template <int dim, int degree>
 void MultiPhysicsBVP<dim,degree>::reinit(){

	 computing_timer_pf.enter_subsection("multiPhysicsBVP: reinitialization");
 	...
 	bool dU_vector_init = false;
 	 ...
 	 for(unsigned int fieldIndex=0; fieldIndex<fields.size(); fieldIndex++){
 		 ...
 		 vectorType_pf *R=residualSet.at(fieldIndex);
 	 ...
 	 computing_timer_pf.leave_subsection("multiPhysicsBVP: reinitialization");
}
