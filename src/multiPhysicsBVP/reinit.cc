...
MatrixFreePDE<dim, degree>::reinit()
{
  ...
  // The member "mpi_communicator" was removed in deal.II version 8.5 but is required
  // before it
  additional_data.tasks_parallel_scheme =
    MatrixFree<dim, double>::AdditionalData::partition_partition;
  // additional_data.tasks_parallel_scheme = MatrixFree<dim,double>::AdditionalData::none;
  // additional_data.tasks_block_size = 1; // This improves performance for small runs,
  // not sure about larger runs
  ...
  for (unsigned int fieldIndex = 0; fieldIndex < fields.size(); fieldIndex++)
    {
      ...
      *U = 0;
      ...
      // Initializing temporary dU vector required for implicit solves of the elliptic
      // equation.
  ...
  for (unsigned int fieldIndex = 0; fieldIndex < fields.size(); fieldIndex++)
    {
      soltransSet.push_back(
        new parallel::distributed::SolutionTransfer<dim, vectorType_pf>(
          *dofHandlersSet_nonconst[fieldIndex]));
    }

  // If remeshing at the zeroth time step, re-apply initial conditions so the starting
  // values are correct on the refined mesh
  if (currentIncrement == 0 && !userInputs.load_grain_structure)
    {
      applyInitialConditions();
    }

  // Ghost the solution vectors. Also apply the Dirichet BC's (if any) on the solution
  // vectors
  for (unsigned int fieldIndex = 0; fieldIndex < fields.size(); fieldIndex++)
    {
