//mesh generation/importing method for multiPhysicsBVP class
#include <fstream>
#include "../../include/multiPhysicsBVP.h"

//generate or import mesh
template <int dim, int degree>
void MultiPhysicsBVP<dim,degree>::mesh(){
  if(userInputs_cp.readExternalMesh){
    //reading external mesh
    pcout << "reading problem mesh\n";
    GridIn<dim> gridin;
    gridin.attach_triangulation(this->triangulation_cp);
    //Read mesh in UCD format generated from Cubit
    std::ifstream extMesh(userInputs_cp.externalMeshFileName);
    ...
    GridOut grid_out;
    grid_out.write_vtk (this->triangulation_cp, out);
    ...
    GridGenerator::subdivided_hyper_rectangle (triangulation2_cp, userInputs_cp.subdivisions, Point<dim>(), Point<dim>(userInputs_cp.span[0],userInputs_cp.span[1],userInputs_cp.span[2]), true);

   //////In the case of element deletion, this section will be active
    if(userInputs_cp.enableElementDeletion){
      unsigned int gID;
      orientations_Mesh.loadOrientations(userInputs_cp.grainIDFile,
              userInputs_cp.headerLinesGrainIDFile,
              userInputs_cp.grainOrientationsFile,
              userInputs_cp.numPts,
              userInputs_cp.span);
      typename Triangulation< dim, dim >::active_cell_iterator cell = triangulation2_cp.begin_active(), endc = triangulation2_cp.end();
      for (; cell!=endc; ++cell) {
              double pnt3[3];
      ...
      cell = triangulation2_cp.begin_active(); endc = triangulation2_cp.end();
      ...
      for (; cell!=endc; ++cell) {
        ...
        materialID=cellOrientationMap_Mesh[cell2];
          ...
          if (materialID ==userInputs_cp.deletionGrainID){
      ...
      sprintf(buffer,
       ...
       if (cells_to_remove.size()>0){
        GridGenerator::create_triangulation_with_removed_cells(triangulation2_cp,cells_to_remove,triangulation_cp);
       }
       else{
        triangulation_cp.copy_triangulation(triangulation2_cp);
       }
    }
    else {
      triangulation_cp.copy_triangulation(triangulation2_cp);
  ...
  //and add those dofs as ghost cells of the other face.
    if(userInputs_cp.enablePeriodicBCs){
      // Set which (if any) faces of the triangulation_cp are periodic
      setPeriodicity_cp();
    }



    triangulation_cp.refine_global (userInputs_cp.meshRefineFactor);
    ...
    if(userInputs_cp.writeMeshToEPS)
      if ((triangulation_cp.n_global_active_cells()<10000) and (Utilities::MPI::n_mpi_processes(mpi_communicator)==1)){
        std::string dir(userInputs_cp.outputDirectory);
        dir+="/";
        ...
        std::ofstream out ((filename).c_str());
        GridOut grid_out;
        grid_out.write_eps (triangulation_cp, out);
