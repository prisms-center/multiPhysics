      ...
      if (currentIncrement == 0)
        {
          ...
          std::cout << "Debug: Inside solve BEFORE solveIncrement (MatrixFreePDE):" << std::endl;
          ...
          for (size_t i = 0; i < solutionSet.size(); ++i)
            {
              if (solutionSet[i] != nullptr)
                {
                  std::cout << "SolutionSet[" << i << "]: size=" << solutionSet[i]->size()
                            << std::endl;
                }
              else
                {
          ...
          std::cout << "Debug: Inside solve AFTER solveIncrement (MatrixFreePDE):" << std::endl;
          std::cout << "Fields size: " << fields.size() << std::endl;
          std::cout << "SolutionSet size: " << solutionSet.size() << std::endl;
          ...
          if (userInputs.outputTimeStepList[currentOutput] == currentIncrement)
            {
              std::cout << "ConstraintsDirichletSet size: "
                        << constraintsDirichletSet.size() << std::endl;
              std::cout << "ConstraintsOtherSet size: " << constraintsOtherSet.size()
                        << std::endl;
              std::cout << "SolutionSet size: " << solutionSet.size() << std::endl;
          ...
          if (userInputs.checkpointTimeStepList[currentCheckpoint] ==
              currentIncrement)
            {
              save_checkpoint();
              currentCheckpoint++;
          ...
          // Increase the current increment from 0 to 1 now that the initial conditions
          // have been output
          currentIncrement++;
          ...
          pcout << "\nTime stepping parameters: timeStep: " << userInputs.dtValue
                ...
                << "  timeIncrements: " << userInputs.totalIncrements_pf << "\n";
        }
      // This is the main time-stepping loop
      // for (; currentIncrement<=userInputs.totalIncrements_pf;
      // ++currentIncrement){ increment current time
      ...
      if (userInputs.outputTimeStepList[currentOutput] == currentIncrement)
        {
          ...
          if (userInputs.print_timing_with_output &&
              currentIncrement < userInputs.totalIncrements_pf)
            {
