//initialization method for multiPhysicsBVP class
...
void MultiPhysicsBVP<dim,degree>::init_cp(){
  std::string line;
  ...
  << triangulation_cp.n_global_active_cells()
  << std::endl
  ...
  if(userInputs_cp.enablePeriodicBCs){
    numberVerticesConstraint=userInputs_cp.numberVerticesConstraint;
    numberEdgesConstraint=userInputs_cp.numberEdgesConstraint;
    numberFacesConstraint=userInputs_cp.numberFacesConstraint;
    ...
    bcDataFile.open(userInputs_cp.Periodic_BCfilename);
    id=0;
    //read data
...
//INDENTATION
    if(userInputs_cp.enableIndentationBCs){
      ...
      std::ifstream IndentationBCfile(userInputs_cp.Indentation_BCfilename);

      //read 5 header lines for documentation for now!!!
      // read 4 data lines for now!!

      if (IndentationBCfile.is_open()){
            KeyPosIndenter.resize(userInputs_cp.indentationKeyFrames);
            ...
            unsigned int lines =userInputs_cp.indentationKeyFrames + 1;
...
//                prevPosIndenter[i] = initPosIndenter[i];
      ...
      if(userInputs_cp.readExternalMesh && false){ // DISABLE FOR IMPORTING BOUNDARIES AS PHYSICAL SURFACES
          // Set which (if any) faces of the triangulation_cp are indentation
          ...
          for (unsigned int i=0; i<dim; ++i) {
            externalMeshParameterBCs(i)=this->userInputs_cp.externalMeshParameter*this->userInputs_cp.span[i];
          }
                  ...
                  for (unsigned int i=0; i<dofs_per_cell; ++i) {
                    if (fe_face_values.shape_value(i, 0)!=0){ //skip cell support points not on face
                      ...
                      for (unsigned int i2=0; i2<dim; ++i2)
                      {
                          ...
                          if (node_BoundaryID[i2] >= (this->userInputs_cp.span[i2]-externalMeshParameterBCs(0))) {
    ...
    if((userInputs_cp.enableSimpleBCs)||(userInputs_cp.enableCyclicLoading)){
      std::ifstream BCfile(userInputs_cp.BCfilename);

      //read data
      if (BCfile.is_open()){
        pcout << "Reading boundary conditions\n";
        //skip header lines
        for (unsigned int i=0; i<userInputs_cp.BCheaderLines; i++) std::getline (BCfile,line);
        for (unsigned int i=0; i<userInputs_cp.NumberofBCs; i++){
    ...
    if(userInputs_cp.enableNodalDisplacementBCs){
      ...
      std::ifstream BCfileNodal(userInputs_cp.nodalDisplacement_BCfilename);
      nodalDisplacement.resize(userInputs_cp.numberOfNodalBCs,std::vector<double>(3,0));
      dofNodalDisplacement.resize(userInputs_cp.numberOfNodalBCs);
      deluNodalDisplacement.resize(userInputs_cp.numberOfNodalBCs);

      //read data
      if (BCfileNodal.is_open()){
        pcout << "Reading Nodal boundary conditions\n";
        for (unsigned int i=0; i<userInputs_cp.numberOfNodalBCs; i++){
...
//    }
    ...
    if(userInputs_cp.useVelocityGrad){
      ...
      for(unsigned int i=0;i<3;i++){
        for(unsigned int j=0;j<3;j++){
          targetVelGrad[i][j] = userInputs_cp.targetVelGrad[i][j];
        }
      }
    }
    ...
    if(userInputs_cp.enableTabularBCs){
      tabularDisplacements.reinit(2*dim*dim,userInputs_cp.tabularBCs_InputStepNumber);
      tabularDisplacements=0;
      //open data file to boundary displacements
      bcDataFile.open(userInputs_cp.Tabular_BCfilename);
      //read data
      if (bcDataFile.is_open()){
        //read data
        id=0;
        while (getline (bcDataFile,line) && id<(userInputs_cp.tabularNumberofBCs)){
          std::stringstream ss(line);
          ss>>faceID>>dof;
          faceDOFConstrained[faceID-1][dof-1]=true;

          for (unsigned int i=0; i<userInputs_cp.tabularBCs_InputStepNumber; i++){
    ...
    if(userInputs_cp.enableNeumannBCs){
      tabularInputNeumannBCs.reinit(userInputs_cp.neumannBCsNumber,userInputs_cp.tabularNeumannBCs_InputStepNumber);
      tabularInputNeumannBCs=0;
      //open data file to boundary displacements
      bcDataFile.open(userInputs_cp.Tabular_NeumannBCfilename);
      //read data
      if (bcDataFile.is_open()){
        //read data
        id=0;
        while (getline (bcDataFile,line) && id<(userInputs_cp.neumannBCsNumber)){
          std::stringstream ss(line);
          for (unsigned int i=0; i<userInputs_cp.tabularNeumannBCs_InputStepNumber; i++){
    ...
    if(userInputs_cp.enableDICpipeline){
      bc_new1.reinit(userInputs_cp.Y_dic,1+2*userInputs_cp.DIC_InputStepNumber);
      bc_new2.reinit(userInputs_cp.Y_dic,1+2*userInputs_cp.DIC_InputStepNumber);
      bc_new3.reinit(userInputs_cp.X_dic,1+2*userInputs_cp.DIC_InputStepNumber);
      bc_new4.reinit(userInputs_cp.X_dic,1+2*userInputs_cp.DIC_InputStepNumber);
      //open data file to boundary displacements
      bcDataFile.open(userInputs_cp.DIC_BCfilename1);
      //read data
      id=0;
      if (bcDataFile.is_open()){
        //read data
        while (getline (bcDataFile,line) && id<(userInputs_cp.Y_dic)){
          std::stringstream ss(line);
          ss >> bc_new1[id][0];
          for (unsigned int i=0; i<userInputs_cp.DIC_InputStepNumber; i++){
            ss >> bc_new1[id][1+i*2];
            ss >> bc_new1[id][2+i*2];
          }
          //cout<<id<<'\t'<<bc_new[id][0]<<'\t'<<bc_new[id][1]<<'\t'<<bc_new[id][2]<<'\n';
      ...
      //open data file to boundary displacements
      bcDataFile.open(userInputs_cp.DIC_BCfilename2);
      //read data
      id=0;
      if (bcDataFile.is_open()){
        //read data
      ...
      //open data file to boundary displacements
      bcDataFile.open(userInputs_cp.DIC_BCfilename3);
      //read data
      id=0;
      if (bcDataFile.is_open()){
        //read data
        while (getline (bcDataFile,line) && id<(userInputs_cp.X_dic)){
          std::stringstream ss(line);
          ss >> bc_new3[id][0];
          for (unsigned int i=0; i<userInputs_cp.DIC_InputStepNumber; i++){
            ss >> bc_new3[id][1+i*2];
      ...
      //open data file to boundary displacements
      bcDataFile.open(userInputs_cp.DIC_BCfilename4);
      //read data
      id=0;
      if (bcDataFile.is_open()){
        //read data
