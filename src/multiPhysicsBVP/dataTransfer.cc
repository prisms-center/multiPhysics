    ...
    FEValues<dim> &fe_values) 
{
    ...
    // Loop over the twin variants
    for (unsigned int i = 0; i < userInputs_cp.numTwinSystems1; i++)
      {
        // Obtaining owned and relevant dofs from PF solution
        const auto &owned_dofs = pf_obj.getDofHandlersSet()[3*i]->locally_owned_dofs();
        IndexSet relevant_dofs;
        DoFTools::extract_locally_relevant_dofs(*pf_obj.getDofHandlersSet()[3*i], relevant_dofs);
        ...
        temp_vector = *pf_obj.getSolutionSet()[3*i];
        temp_vector.update_ghost_values(); // Ensure ghost values are updated

        // Create interpolation function from PF solutionSet
        Functions::FEFieldFunction<dim, vectorType_pf> fe_function_1(*pf_obj.getDofHandlersSet()[3*i], temp_vector);
            ...
            if (cell->is_locally_owned())
              {
                ...
                for (unsigned int q = 0; q < quadrature.size(); ++q)
                  {
                    twinfraction_iter1[local_index][q][i] = solution_values_cp[q];
                  }
              }
          }
      }
...
void MultiPhysicsBVP<dim, degree>::interpolate_twin_energy(
    ...
    const DoFHandler<dim> &dofHandler_Scalar) 
{
    pf_obj.pcout << "Interpolating twin energy\n" << std::endl;
    // Obtaining own and locally_relevant dofs from CPFE solution
    IndexSet own_dofs = dofHandler_Scalar.locally_owned_dofs();
    IndexSet locally_relevant_dofs;
    DoFTools::extract_locally_relevant_dofs(dofHandler_Scalar, locally_relevant_dofs);

    // Loop over each twin system
    for (unsigned int i = 0; i < userInputs_cp.numTwinSystems1; i++)
      {
        ...
        temp_vector_cp = *postFieldsWithGhosts[26 - i];
        ...
        pf_obj.pcout << "Created fe_function_3 object for twin variant " << (i + 1) << std::endl;

        //Obtaining owned dofs of pf_obj.getDofHandlersSet()[2]
        const auto &owned_dofs = pf_obj.getDofHandlersSet()[3*i + 2]->locally_owned_dofs();

        //Obtaining relevant dofs of pf_obj.getDofHandlersSet()[3*i + 2]
        IndexSet relevant_dofs;
        DoFTools::extract_locally_relevant_dofs(*pf_obj.getDofHandlersSet()[3*i + 2], relevant_dofs);
        //Declare temporary vector solution_pf
        vectorType_pf solution_pf;

        //Reinit solution_pf
        solution_pf.reinit(owned_dofs, relevant_dofs, MPI_COMM_WORLD);
        pf_obj.pcout << "Reinit of solution_pf successful for twin variant " << (i + 1) << std::endl;

        // Interpolate values from CPFE into solution_pf using fe_function_3
        VectorTools::interpolate(*pf_obj.getDofHandlersSet()[3*i + 2], fe_function_3, 
                                    solution_pf); 

        //Copy values solution_pf into *pf_obj.getSolutionSet()[2]
        *pf_obj.getSolutionSet()[3*i + 2] = solution_pf;
      }
