//outputResults() method for MultiPhysicsBVP class
...
void MultiPhysicsBVP<dim,degree>::outputResults() {
    //log time
    computing_timer_pf.enter_subsection("multiPhysicsBVP: output");
    ...
    if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0) {
        if (!create_directory_if_not_exists(userInputs_pf.output_directory_pf)) {
            std::cerr << "Error creating output directory: " << userInputs_pf.output_directory_pf << std::endl;
    ...
    cycleAsString << std::setw(std::floor(std::log10(userInputs_pf.totalIncrements_pf)) + 1)
                << std::setfill('0') << currentIncrement_pf;

    // Create directory prefix (with trailing slash if needed)
    std::string prefix = userInputs_pf.output_directory_pf;
    if (!prefix.empty() && prefix.back() != '/')
        prefix += "/";

    char baseFileName[200], vtuFileName[200];
    snprintf(baseFileName, sizeof(baseFileName), "%s%s-%s",
            prefix.c_str(),
            userInputs_pf.output_file_name.c_str(),
            cycleAsString.str().c_str());

    snprintf(vtuFileName, sizeof(vtuFileName), "%s.%u.%s",
            baseFileName,
            Utilities::MPI::this_mpi_process(MPI_COMM_WORLD),
            userInputs_pf.output_file_type.c_str());

    // Write to file in either vtu or vtk format
    if (userInputs_pf.output_file_type == "vtu") {
        dealii::DataOutBase::VtkFlags flags;
        flags.time = currentTime_pf;
        flags.cycle = currentIncrement_pf;
        flags.print_date_and_time = true;
        data_out.set_flags(flags);

        if (userInputs_pf.output_vtu_per_process) {
            ...
            if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0) {
                ...
                for (unsigned int i = 0; i < Utilities::MPI::n_mpi_processes(MPI_COMM_WORLD); ++i) {
                    ...
                    snprintf(vtuProcFileName, sizeof(vtuProcFileName), "%s%s-%s.%u.%s",
                            prefix.c_str(),
                            userInputs_pf.output_file_name.c_str(),
                            cycleAsString.str().c_str(),
                            i,
                ...
                snprintf(pvtuFileName, sizeof(pvtuFileName), "%s.p%s",
                        baseFileName,
                        userInputs_pf.output_file_type.c_str());
            ...
            snprintf(svtuFileName, sizeof(svtuFileName), "%s.%s",
                    baseFileName,
                    userInputs_pf.output_file_type.c_str());
  ...
  //log time
  computing_timer_pf.leave_subsection("multiPhysicsBVP: output");
}
