      ...
      if (userInputs_pf.output_vtu_per_process){
          ...
          if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0){
            ...
            for (unsigned int i=0;i<Utilities::MPI::n_mpi_processes (MPI_COMM_WORLD); ++i) {
            	...
            	snprintf(vtuProcFileName, sizeof(vtuProcFileName),"%s-%s.%u.%s", userInputs_pf.output_file_name.c_str(),cycleAsString.str().c_str(),i,userInputs_pf.output_file_type.c_str());
            	filenames.push_back (vtuProcFileName);
            }
            char pvtuFileName[100];
            snprintf(pvtuFileName, sizeof(pvtuFileName),"%s.p%s", baseFileName, userInputs_pf.output_file_type.c_str());
