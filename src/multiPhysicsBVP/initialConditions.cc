  ...
  if (userInputs.load_grain_structure)
    {
      ...
      // Delete grains with very small radii that correspond to a single interfacial
      // element
      ...
      // field_i^(n+1)=field_i^(n)+D*dt*Laplacian(field_i^(n))
      // Old dt_for_smoothing
      // double dt_for_smoothing =
      // dealii::GridTools::minimal_cell_diameter(triangulation)/1000.0; NEW selection of
      // dt_for_smoothing based on dt_max=(dx^2)/2*dim*D, where D=1, addording to the von
      // Neumann stability condition We chose dt_for_smoothing=0.25*dt_max
              ...
              if (fieldIndex == userInputs.variables_for_remapping.at(op_list_index))
                {
                  for (unsigned int cycle = 0;
                       ...
                       cycle++)
                    {
                      // Calculates the Laplace RHS and stores the information in
                      // residualSet
          ...
          if (userInputs.load_ICs[var_index] == false)
          ...
          else
            {
              ...
              if (userInputs.var_type[var_index] == SCALAR)
                ...
                {
                  std::cout
                    << "PRISMS-PF Error: Cannot load vector fields. Loading initial "
                       "conditions from file is currently limited to scalar fields"
                    << std::endl;
                }
            }
...
//
//  InitialConditionPFieldVec (const unsigned int _index, ScalarField2D &_inputField) :
//  Function<dim>(1), index(_index), inputField(_inputField) {}
//
