...
void MultiPhysicsBVP<dim,degree>::applyInitialConditions_pf(){

    if (userInputs_pf.load_grain_structure){
        // Create the dummy field
        vectorType_pf grain_index_field;

        // Clear the order parameter fields
        unsigned int op_list_index = 0;
        for (unsigned int var_index=0; var_index < userInputs_pf.number_of_variables; var_index++){
            if (op_list_index < userInputs_pf.variables_for_remapping.size()){
                if (var_index == userInputs_pf.variables_for_remapping.at(op_list_index)){
        ...
        std::string filename = userInputs_pf.grain_structure_filename;
        filename += ".vtk";

        body.read_vtk(filename);
        ScalarField &id_field = body.find_scalar_field(userInputs_pf.grain_structure_variable_name);
        ...
        for (unsigned int id=min_id; id<max_id+1; id++){
            ...
            flood_filler.calcGrainSets(*FESet.at(scalar_field_index), *dofHandlersSet_nonconst.at(scalar_field_index), &grain_index_field, (double)id - userInputs_pf.order_parameter_threshold, (double)id + userInputs_pf.order_parameter_threshold, 0, grain_sets_single_id);
        ...
        for (unsigned int g=0; g<simplified_grain_representations.size(); g++){
            if (simplified_grain_representations.at(g).getRadius() < userInputs_pf.min_radius_for_loading_grains){
        ...
        order_parameter_remapper.remap_from_index_field(simplified_grain_representations, &grain_index_field, solutionSet, *dofHandlersSet_nonconst.at(scalar_field_index), FESet.at(scalar_field_index)->dofs_per_cell, userInputs_pf.buffer_between_grains);
        ...
        //double dt_for_smoothing = dealii::GridTools::minimal_cell_diameter(triangulation_pf)/1000.0;
        ...
        double min_dx=dealii::GridTools::minimal_cell_diameter(triangulation_pf)/(1.0*degree*std::sqrt(1.0*dim));
        ...
        for(unsigned int fieldIndex=0; fieldIndex<fields.size(); fieldIndex++){
            if (op_list_index < userInputs_pf.variables_for_remapping.size()){
                if ( fieldIndex == userInputs_pf.variables_for_remapping.at(op_list_index)){
                    ...
                    for (unsigned int cycle=0; cycle<userInputs_pf.num_grain_smoothing_cycles; cycle++){
    ...
    for (unsigned int var_index=0; var_index < userInputs_pf.number_of_variables; var_index++){
        ...
        if (!is_remapped_op || !userInputs_pf.load_grain_structure){

            if (userInputs_pf.load_ICs[var_index] == false){
                pcout << "Applying non-PField initial condition...\n";

                if (userInputs_pf.var_type[var_index] == SCALAR){
                    ...
                    VectorTools::interpolate (*dofHandlersSet[var_index], InitialConditionVector<dim,degree>(var_index,userInputs_pf,this), *solutionSet[var_index]);
                }
            }
                ...
                // Create the filename of the the file to be loaded
                std::string filename;
                if (userInputs_pf.load_parallel_file[var_index] == false){
                    filename = userInputs_pf.load_file_name[var_index] + ".vtk";
                }
                else {
                    int proc_num = Utilities::MPI::this_mpi_process(MPI_COMM_WORLD);
                    std::ostringstream conversion;
                    conversion << proc_num;
                    filename = userInputs_pf.load_file_name[var_index] + "." + conversion.str() + ".vtk";
                }

                // Load the data from the file using a PField
                body.read_vtk(filename);
                ScalarField &conc = body.find_scalar_field(userInputs_pf.load_field_name[var_index]);
