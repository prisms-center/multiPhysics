...
MatrixFreePDE<dim, degree>::adaptiveRefineCriterion()
{
  ...
  for (unsigned int field_index = 0;
       ...
       field_index++)
    {
      ...
      else if (userInputs.refinement_criteria[field_index].criterion_type ==
                 GRADIENT ||
               userInputs.refinement_criteria[field_index].criterion_type ==
                 VALUE_AND_GRADIENT)
        {
  ...
  for (unsigned int fieldIndex = 0; fieldIndex < fields.size(); fieldIndex++)
    {
      ...
      // residualSet is cleared in reinit(), so I don't see the reason for the pointer
      // assignment Changing to the new version has no impact on the solution.
