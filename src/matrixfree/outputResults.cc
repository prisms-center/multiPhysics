...
//output results for cpfe fields
template <int dim, int degree>
void MatrixFreePDE<dim,degree>::outputResults_cp() {
  //log time
  computing_timer.enter_subsection("matrixFreePDE: output");
  ...
  //mark field as scalar/vector
  std::vector<DataComponentInterpretation::DataComponentInterpretation> dataType \
  (fields[0].numComponents,				\
  (fields[0].type==SCALAR ?				\
  DataComponentInterpretation::component_is_scalar:		\
  DataComponentInterpretation::component_is_part_of_vector));
  //add field to data_out
  std::vector<std::string> solutionNames (fields[0].numComponents, "cpfe_field");
  data_out.add_data_vector(*dof_handler_cp, *solution_cp, solutionNames, dataType);
/*
  // Test section for outputting postprocessed fields
  // Currently there are hacks in place, using the matrixFreeObject, invM, constraints, and DoFHandler as the primary variables
  if (userInputs.postProcessingRequired){
  ...
  */
  ...
  cycleAsString << std::setw(std::floor(std::log10(userInputs.totalIncrements))+1) << std::setfill('0') << currentIncrement;
  char baseFileName[100], vtuFileName[100];
  snprintf(baseFileName, sizeof(baseFileName),"%s-cp-%s", userInputs.output_file_name.c_str(),cycleAsString.str().c_str());
  snprintf(vtuFileName, sizeof(vtuFileName),"%s.%u.%s", baseFileName,Utilities::MPI::this_mpi_process(MPI_COMM_WORLD),userInputs.output_file_type.c_str());
