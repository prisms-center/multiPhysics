...
template <int dim>
void continuumPlasticity<dim>::writeQuadratureOutput(std::string _outputDirectory, unsigned int _currentIncrement_cp){
	...
	std::ofstream file((dir + fileName + std::to_string(_currentIncrement_cp)+fileExtension).c_str());
	char buffer[200];
	if (file.is_open()) {
	...
	if (dealii::Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0) {
		...
		std::ofstream file2((dir + fileName2 + std::to_string(_currentIncrement_cp)+fileExtension).c_str());
		for (unsigned int proc = 0; proc<dealii::Utilities::MPI::n_mpi_processes(MPI_COMM_WORLD); proc++) {
			...
			std::ofstream file3((dir  + fileName3 + std::to_string(_currentIncrement_cp)+fileExtension).c_str(), std::ofstream::in);
			file2 << file3.rdbuf();
			//delete file from processor proc
			remove((dir + fileName3 + std::to_string(_currentIncrement_cp)+fileExtension).c_str());
		}
		file2.close();
	}
}
