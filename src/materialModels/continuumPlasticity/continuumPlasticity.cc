...
//constructor
template <int dim>
continuumPlasticity<dim>::continuumPlasticity(userInputParameters_pf<dim> _userInputs_pf, userInputParameters_cp & _userInputs_cp)
:
  ...
  enhStrain(this->FE,this->pcout, _userInputs_cp)
{
  ...
  MultiPhysicsBVP<dim,1>::postprocessed_solution_names.push_back("alpha");
  ...
  MultiPhysicsBVP<dim,1>::numPostProcessedFieldsAtCellCenters=1; //grainID
}
