...
void crystalPlasticity<dim>::getElementalValues(FEValues<dim>& fe_values,
	...
	Vector<double>&     elementalResidual)
	{
		...
		typename DoFHandler<dim>::active_cell_iterator cell(& this->triangulation_cp,
			...
			///////Applying NeumannBCs/////////
			if (this->userInputs_cp.enableNeumannBCs){
				QGauss<dim-1>  quadrature_face(this->userInputs_cp.quadOrder);
				...
				double traction_Neumann,currentTime_cp;
				bool flag_Neumann;
				...
				for (unsigned int neumannBCsNumber=0;neumannBCsNumber<this->userInputs_cp.neumannBCsNumber;++neumannBCsNumber){
					neumannBCsBoundaryID=this->userInputs_cp.neumannBCsBoundaryID[neumannBCsNumber];
					dof_NeumannBCs=this->userInputs_cp.dofNeumannBCs[neumannBCsNumber];
					currentTime_cp=this->delT*(this->currentIncrement_cp+1);					
					traction_Neumann=this->tabularInputNeumannBCs[neumannBCsNumber][this->timeCounter_Neumann-1]+(-this->tabularInputNeumannBCs[neumannBCsNumber][this->timeCounter_Neumann-1]+this->tabularInputNeumannBCs[neumannBCsNumber][this->timeCounter_Neumann])/(this->userInputs_cp.tabularTimeNeumannBCs[this->timeCounter_Neumann]-this->userInputs_cp.tabularTimeNeumannBCs[this->timeCounter_Neumann-1])*(currentTime_cp-this->userInputs_cp.tabularTimeNeumannBCs[this->timeCounter_Neumann-1]);
							...
							for (unsigned int i=0; i<dofs_per_cell; ++i) {
								if (fe_face_values.shape_value(i, 0)!=0){
									...
									switch (neumannBCsBoundaryID){
										...
										break;
										case 2:
										if (node_Neumann[0] >= (this->userInputs_cp.span[0]-externalMeshParameterBCs(0)))
										{//pcout<<i<<" "<<dof<<std::endl;
										flag_Neumann=true;}
										break;
										case 3:
										...
										break;
										case 4:
										if (node_Neumann[1] >= (this->userInputs_cp.span[1]-externalMeshParameterBCs(1)))
										{//pcout<<i<<" "<<dof<<std::endl;
										flag_Neumann=true; }
										break;
										case 5:
										...
										break;
										case 6:
										if (node_Neumann[2] >= (this->userInputs_cp.span[2]-externalMeshParameterBCs(2)))
										{//pcout<<i<<" "<<dof<<std::endl;
										flag_Neumann=true;}
						 			}
						 			break;
