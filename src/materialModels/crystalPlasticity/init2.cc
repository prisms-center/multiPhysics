...
void crystalPlasticity<dim>::init2(unsigned int num_quad_points)
{
  ...
  for (unsigned int i=0;i<n_twin_systems;i++){
    for (unsigned int j = 0;j < (n_twin_systems / 2) + 1;j++) {
      s0_init(i + n_slip_systemsWOtwin + n_slip_systems*j) = this->userInputs_cp.initialSlipResistanceTwin1[i]*1000;
    }
  }
  ...
  double s0_twin=this->userInputs_cp.initialSlipResistanceTwin1[n_twin_systems-1]*1000;
