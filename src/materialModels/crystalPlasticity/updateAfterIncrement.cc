...
void crystalPlasticity<dim>::updateAfterIncrement()
{
  ...
  FullMatrix<double> F_lastIter(dim,dim),deltaF(dim,dim);
	...
	if (this->userInputs_cp.flagUserDefinedAverageOutput){
		userDefinedAverageOutput.reinit(this->userInputs_cp.numberUserDefinedAverageOutput);
		userDefinedAverageOutput=0;
		local_userDefinedAverageOutput.reinit(this->userInputs_cp.numberUserDefinedAverageOutput);
...
//Trapezoidal integration rule
        ...
        FirstPiolaStress[cellID][q]=P;
				...
				if (!this->userInputs_cp.enableAdvancedTwinModel){
					...
					if (TotaltwinvfK[cellID][q]>=this->userInputs_cp.criteriaTwinVisual){
				...
				if (this->userInputs_cp.writeOutput){
					...
					this->postprocessValues(cellID, q, 3, 0) = 0;//energy[cellID][q][1];
					this->postprocessValues(cellID, q, 4, 0) = 0;//energy[cellID][q][2];
					this->postprocessValues(cellID, q, 5, 0) = 0;//energy[cellID][q][3];
					this->postprocessValues(cellID, q, 6, 0) = 0;//energy[cellID][q][4];
					this->postprocessValues(cellID, q, 7, 0) = 0;//energy[cellID][q][5];
					this->postprocessValues(cellID, q, 8, 0) = 0;//energy[cellID][q][6];
				...
				if (CheckBufferRegion==1) {
					...
					for(unsigned int i=0;i<this->userInputs_cp.numSlipSystems1;i++){
...
////////User Defined Variables for visualization outputs for cell_centers (outputoutputCellCenters_Var1 to outputoutputCellCenters_Var24)////////
	...
	if (this->userInputs_cp.tabularOutput){
		...
		for(unsigned int i=0;i<this->userInputs_cp.tabularTimeOutput.size();i++){
			tabularTimeInputInc[i]=tabularTimeInputInc[i]/this->delT;
		}

		tabularTimeInputIncInt.resize(this->userInputs_cp.tabularTimeOutput.size(),0);
	...
	if (this->userInputs_cp.writeQuadratureOutput) {
		if (((!this->userInputs_cp.tabularOutput)&&((this->currentIncrement_cp+1)%this->userInputs_cp.skipQuadratureOutputSteps == 0))||((this->userInputs_cp.tabularOutput)&& (std::count(tabularTimeInputIncInt.begin(), tabularTimeInputIncInt.end(), (this->currentIncrement_cp+1))==1))){
			...
			writeQuadratureOutput(this->userInputs_cp.outputDirectory, this->currentIncrement_cp);
		}
	}

	if (this->userInputs_cp.writeGrainAveragedOutput) {
		if (((!this->userInputs_cp.tabularOutput)&&((this->currentIncrement_cp+1)%this->userInputs_cp.skipGrainAveragedOutputSteps == 0))||((this->userInputs_cp.tabularOutput)&& (std::count(tabularTimeInputIncInt.begin(), tabularTimeInputIncInt.end(), (this->currentIncrement_cp+1))==1))){
			//loop over elements
			unsigned int numberOfGrainAverageDataOutput=this->userInputs_cp.numberOfGrainAverageDataOutput;
			cellID=0;
	...
	if (this->userInputs_cp.flagUserDefinedAverageOutput){
		for(unsigned int i=0;i<this->userInputs_cp.numberUserDefinedAverageOutput;i++){
