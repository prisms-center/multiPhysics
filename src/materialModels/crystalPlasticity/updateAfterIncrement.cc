...
template <int dim> void crystalPlasticity<dim>::updateAfterIncrement() {
  ...
  if (this->userInputs_cp.flagUserDefinedAverageOutput) {
    userDefinedAverageOutput.reinit(
        this->userInputs_cp.numberUserDefinedAverageOutput);
    userDefinedAverageOutput = 0;
    local_userDefinedAverageOutput.reinit(
        this->userInputs_cp.numberUserDefinedAverageOutput);
  ...
  for (; cell != endc; ++cell) {
    if (cell->is_locally_owned()) {
      ...
      if (this->userInputs_cp.flagBufferLayer) {
        ...
        if ((pnt2[dimBuffer] >= lowerBuffer) &&
            (pnt2[dimBuffer] <= upperBuffer)) {
          CheckBufferRegion = 1;
        } else {
          CheckBufferRegion = 0;
        }
      ...
      for (unsigned int q = 0; q < num_quad_points; ++q) {
        ...
        // Update strain, stress, and tangent for current time step/quadrature
        // point
        calculatePlasticity(cellID, q, 0);

        FullMatrix<double> temp, temp3, temp4, C_tau(dim, dim), E_tau(dim, dim),
            b_tau(dim, dim);
        Vector<double> temp2;
        temp.reinit(dim, dim);
        temp = 0.0;
        ...
        for (unsigned int i = 0; i < dim; ++i) {
          for (unsigned int j = 0; j < dim; ++j) {
            ...
            workDensity_Element1_Tr += ((P[i][j] + P_LastIter[i][j]) / 2) *
        ...
        if (this->userInputs_cp.enableAdvRateDepModel) {
          for (unsigned int i = 0; i < dim; i++) {
            for (unsigned int j = 0; j < dim; j++) {
              TinterStress_diff[cellID][q][i][j] =
                  T_inter[i][j] - TinterStress[cellID][q][i][j];
        ...
        if (!this->userInputs_cp.enableAdvancedTwinModel) {
          ...
          if (TotaltwinvfK[cellID][q] >=
              this->userInputs_cp.criteriaTwinVisual) {
        ...
        if (this->userInputs_cp.writeOutput) {
          ...
          this->postprocessValues(cellID, q, 4, 0) =
              this->twinfraction_iter1[cellID][q][0];
          this->postprocessValues(cellID, q, 5, 0) = energy[cellID][q][2];
        ...
        if (CheckBufferRegion == 1) {
          ...
          for (unsigned int i = 0; i < this->userInputs_cp.numTwinSystems1;
               i++) {
            local_F_r =
                local_F_r + twinfraction_iter[cellID][q][i] * fe_values.JxW(q);
          }
          ...
          for (unsigned int i = 0; i < this->userInputs_cp.numSlipSystems1;
               i++) {
            local_F_s =
          ...
          if (this->userInputs_cp.flagUserDefinedAverageOutput) {
            ...
            /// "+0" should be substituted by "+targetVariable", where
            /// targetVariable is the variable you want to plot the average
            /// value.
            // Also, the first equation should be copy and paste depending on
            // the number of variables you want to output, and the integer in
            // paranthesis should be updated accordingly. The maximum number of
            // lines (output variables) are defined in the input file as "set
            // Number of Output Userdefined Average Variable".
            local_userDefinedAverageOutput(0) =
                local_userDefinedAverageOutput(0) + 0;
          }
        }
      ...
      if (this->userInputs_cp.writeOutput) {
        ...
        ////////User Defined Variables for visualization outputs for
        /// cell_centers (outputoutputCellCenters_Var1 to
        /// outputoutputCellCenters_Var24)////////
  ...
  // Update the history variables when convergence is reached for the current
  // increment
  Fe_conv = Fe_iter;
  Fp_conv = Fp_iter;
  ...
  if (this->userInputs_cp.tabularOutput) {

    tabularTimeInputInc = this->userInputs_cp.tabularTimeOutput;
    for (unsigned int i = 0; i < this->userInputs_cp.tabularTimeOutput.size();
         i++) {
      tabularTimeInputInc[i] = tabularTimeInputInc[i] / this->delT;
    }

    tabularTimeInputIncInt.resize(this->userInputs_cp.tabularTimeOutput.size(),
                                  0);
    /// Converting to an integer always rounds down, even if the fraction part
    /// is 0.99999999.
  ...
  if (this->userInputs_cp.writeQuadratureOutput) {
    if (((!this->userInputs_cp.tabularOutput) &&
         ((this->currentIncrement_cp + 1) %
              this->userInputs_cp.skipQuadratureOutputSteps ==
          0)) ||
        ((this->userInputs_cp.tabularOutput) &&
         (std::count(tabularTimeInputIncInt.begin(),
                     tabularTimeInputIncInt.end(),
                     (this->currentIncrement_cp + 1)) == 1))) {
      ...
      writeQuadratureOutput(this->userInputs_cp.outputDirectory,
                            this->currentIncrement_cp);
    }
  }

  if (this->userInputs_cp.writeGrainAveragedOutput) {
    if (((!this->userInputs_cp.tabularOutput) &&
         ((this->currentIncrement_cp + 1) %
              this->userInputs_cp.skipGrainAveragedOutputSteps ==
          0)) ||
        ((this->userInputs_cp.tabularOutput) &&
         (std::count(tabularTimeInputIncInt.begin(),
                     tabularTimeInputIncInt.end(),
                     (this->currentIncrement_cp + 1)) == 1))) {
      // loop over elements
      unsigned int numberOfGrainAverageDataOutput =
          this->userInputs_cp.numberOfGrainAverageDataOutput;
      cellID = 0;
      FullMatrix<double> grainAveragaData(orientations.numberOfGrains,
                                          numberOfGrainAverageDataOutput);
      FullMatrix<double> global_grainAveragaData(
          orientations.numberOfGrains, numberOfGrainAverageDataOutput);
          ...
          if (CheckBufferRegion == 1) {
            ...
            for (unsigned int q = 0; q < num_quad_points; ++q) {
              ...
              if (temp.size() != numberOfGrainAverageDataOutput) {
                this->pcout << "The size of numberOfGrainAverageDataOutput "
                               "defined in prm.prm is not correct\n";
                exit(1);
              }

              for (unsigned int i = 0; i < numberOfGrainAverageDataOutput;
                   i++) {
                grainAveragaData(grainID, i) =
                    grainAveragaData(grainID, i) + temp[i];
              }
            }
          }
          cellID++;
      ...
      for (unsigned int i = 0; i < orientations.numberOfGrains; i++) {
        for (unsigned int j = 0; j < numberOfGrainAverageDataOutput; j++) {
          global_grainAveragaData[i][j] = Utilities::MPI::sum(
              grainAveragaData[i][j], this->mpi_communicator);
        }
      }
      ...
      for (unsigned int i = 0; i < orientations.numberOfGrains; i++) {
        if (fabs(global_grainAveragaData[i][0]) > 1e-15) {
          for (unsigned int j = 1; j < numberOfGrainAverageDataOutput; j++) {
            global_grainAveragaData[i][j] =
                global_grainAveragaData[i][j] / global_grainAveragaData[i][0];
          }
        }
      }

      std::string dir_GrainAverage(this->userInputs_cp.outputDirectory);

      if (Utilities::MPI::this_mpi_process(this->mpi_communicator) == 0) {
        dir_GrainAverage += "/";
        std::ofstream outputFile_GrainAverage;
        dir_GrainAverage += std::string("GrainAverage");
        dir_GrainAverage += std::to_string(this->currentIncrement_cp);
        dir_GrainAverage += std::string(".csv");
        outputFile_GrainAverage.open(dir_GrainAverage.c_str(),
                                     std::fstream::app);
        for (unsigned int i = 0; i < orientations.numberOfGrains; i++) {
          for (unsigned int j = 0; j < numberOfGrainAverageDataOutput - 1;
               j++) {
            outputFile_GrainAverage << std::setprecision(9)
                                    << global_grainAveragaData[i][j] << ",";
          }
          outputFile_GrainAverage
              << std::setprecision(9)
              << global_grainAveragaData[i][numberOfGrainAverageDataOutput - 1];
          outputFile_GrainAverage << '\n';
        }
  ...
  for (unsigned int i = 0; i < dim; i++) {
    for (unsigned int j = 0; j < dim; j++) {
      global_strain[i][j] =
          Utilities::MPI::sum(local_strain[i][j], this->mpi_communicator) /
          microvol;
      global_stress[i][j] =
          Utilities::MPI::sum(local_stress[i][j], this->mpi_communicator) /
          microvol;
    }
  }
  ...
  if (this->userInputs_cp.flagUserDefinedAverageOutput) {
    for (unsigned int i = 0;
         i < this->userInputs_cp.numberUserDefinedAverageOutput; i++) {
      userDefinedAverageOutput(i) =
          Utilities::MPI::sum(local_userDefinedAverageOutput(i),
  ...
  std::string dir(this->userInputs_cp.outputDirectory);
  if (Utilities::MPI::this_mpi_process(this->mpi_communicator) == 0) {
    dir += "/";
    std::ofstream outputFile;
    dir += std::string("stressstrain.txt");

    if (this->currentIncrement_cp == 0) {
      outputFile.open(dir.c_str());
      outputFile << "Exx" << '\t' << "Eyy" << '\t' << "Ezz" << '\t' << "Eyz"
                 << '\t' << "Exz" << '\t' << "Exy" << '\t' << "Txx" << '\t'
                 << "Tyy" << '\t' << "Tzz" << '\t' << "Tyz" << '\t' << "Txz"
                 << '\t' << "Txy" << '\t' << "TwinRealVF" << '\t' << "TwinMade"
                 << '\t' << "SlipTotal";
    ...
    outputFile << global_strain[0][0] << '\t' << global_strain[1][1] << '\t'
               << global_strain[2][2] << '\t' << global_strain[1][2] << '\t'
               << global_strain[0][2] << '\t' << global_strain[0][1] << '\t'
               << global_stress[0][0] << '\t' << global_stress[1][1] << '\t'
               << global_stress[2][2] << '\t' << global_stress[1][2] << '\t'
               << global_stress[0][2] << '\t' << global_stress[0][1] << '\t'
               << F_r << '\t' << F_e << '\t' << F_s;
    if (this->userInputs_cp.enableIndentationBCs) {
  ...
  //	  Functions::FEFieldFunction<dim,vectorType_cp> fe_function_1
  //(this->dofHandler, this->postFieldsWithGhosts[field]);
  //	}
}

//------------------------------------------------------------------------
template <int dim>
void crystalPlasticity<dim>::rod2quat(Vector<double> &quat,
                                      Vector<double> &rod) {
...
//------------------------------------------------------------------------
template <int dim>
void crystalPlasticity<dim>::quatproduct(Vector<double> &quatp,
                                         Vector<double> &quat2,
                                         Vector<double> &quat1)
// R(qp) = R(q2)R(q1)
{
  ...
  quatp(0) = (a * b) - dot1;
  quatp(1) =
      a * quat1(1) + b * quat2(1) + quat2(2) * quat1(3) - quat1(2) * quat2(3);
  quatp(2) =
      a * quat1(2) + b * quat2(2) - quat2(1) * quat1(3) + quat1(1) * quat2(3);
  quatp(3) =
      a * quat1(3) + b * quat2(3) + quat2(1) * quat1(2) - quat1(1) * quat2(2);
  if (quatp(0) < 0) {
    quatp(0) = -quatp(0);
    quatp(1) = -quatp(1);
    quatp(2) = -quatp(2);
...
//------------------------------------------------------------------------
template <int dim>
void crystalPlasticity<dim>::quat2rod(Vector<double> &quat,
                                      Vector<double> &rod) {
  if (fabs(quat(0)) < 1e-4) {
    quat(0) = 1e-4;
  }
