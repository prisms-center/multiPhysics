...
void MatrixFreePDE<dim,degree>::save_checkpoint(){
    computing_timer_pf.enter_subsection("matrixFreePDE: save_checkpoint");
    ...
    // save Triangulation and Solution vectors:
    {
        ...
        // Finally, save the triangulation_pf and the solutionTransfer objects
    ...
    computing_timer_pf.leave_subsection("matrixFreePDE: save_checkpoint");
...
template <int dim, int degree>
void MatrixFreePDE<dim,degree>::load_checkpoint_triangulation_pf(){
    ...
    {
        AssertThrow(false, ExcMessage("PRISMS-PF Error: Cannot open snapshot mesh file or read the triangulation_pf stored there."));
    }
