...
// =================================================================================
template <int dim, int degree>
void MatrixFreePDE<dim,degree>::applyDirichletBCs_pf(){
...
template <int dim, int degree>
void MatrixFreePDE<dim,degree>::setPeriodicity_pf(){
...
template <int dim, int degree>
void MatrixFreePDE<dim,degree>::setPeriodicityConstraints_pf(AffineConstraints<double>* constraints, const DoFHandler<dim>* dof_handler) const {
	// First, get the variable index of the current field
		unsigned int starting_BC_list_index = 0;
		for (unsigned int i=0; i<currentFieldIndex; i++){
			if (userInputs_pf.var_type[i] == SCALAR){
