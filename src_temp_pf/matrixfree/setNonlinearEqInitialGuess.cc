...
void MatrixFreePDE<dim,degree>::setNonlinearEqInitialGuess(){
    ...
    //log time
    computing_timer_pf.enter_subsection("matrixFreePDE: setNonlinearEqInitialGuess");
    Timer time;
    char buffer[200];
        ...
        if ( (userInputs_pf.var_eq_type[fieldIndex] == TIME_INDEPENDENT) && userInputs_pf.var_nonlinear[fieldIndex] && userInputs_pf.nonlinear_solver_parameters.getLaplaceInitializationFlag(fieldIndex)){
            ...
            try{
                if (fields[fieldIndex].type == SCALAR){
                    dU_scalar=0.0;
                    solver.solve_pf(*this, dU_scalar, *residualSet[fieldIndex], IdentityMatrix(solutionSet[fieldIndex]->size()));
                }
                else {
                    dU_vector=0.0;
                    solver.solve_pf(*this, dU_vector, *residualSet[fieldIndex], IdentityMatrix(solutionSet[fieldIndex]->size()));
                }
            }
            catch (...) {
    ...
    //log time
    computing_timer_pf.leave_subsection("matrixFreePDE: setNonlinearEqInitialGuess");
...
void MatrixFreePDE<dim,degree>::computeLaplaceRHS(unsigned int fieldIndex){
  //log time
  computing_timer_pf.enter_subsection("matrixFreePDE: computeLaplaceRHS");
  ...
  //end log
  computing_timer_pf.leave_subsection("matrixFreePDE: computeLaplaceRHS");
}
