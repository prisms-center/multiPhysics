        ...
        dealii::Vector<double> &vector_IC)
{
    ...
    double nX, nY;
    double interface_width = del0 * 0.5; // Start with a too-narrow interface width
    scalar_IC = 0;
    ...
    if (index==0)
      {
        ...
        scalar_IC = 0.5*(1.0-std::tanh((dist-edist)/(regval + 1.0*interface_width*std::sqrt(edist/a0))));
